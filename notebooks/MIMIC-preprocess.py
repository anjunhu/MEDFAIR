...
for TARGET_ATTRIBUTE in ['Cardiomegaly', 'Edema', 'Pleural Effusion']:
        # read dataframes
        ...
        if VERBOSE: print('train size', len(merged_df_train), 'val size', len(merged_df_valid))

        # A peek at post-binarization training set P(A), P(Y), P(Y, A)
        for sa in range(5):
            df_sa = np.where(merged_df_train['Age_multi'] == sa)
            df_y0 = np.where((merged_df_train['binaryLabel']==0) & (merged_df_train['Age_multi'] == sa))
            df_y1 = np.where((merged_df_train['binaryLabel']==1) & (merged_df_train['Age_multi'] == sa))
